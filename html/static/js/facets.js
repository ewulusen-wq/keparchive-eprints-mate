(function(){
  ...
  function initLightbox(){
    // Collect all images from search results
    const images = Array.from(document.querySelectorAll('.ep_search_result img'));
    if(images.length === 0) return;

    // Create carousel modal
    const modal = document.createElement('div');
    modal.id = 'maeLightbox';
  ...
  document.addEventListener('DOMContentLoaded', function(){
    try{ initFacets(); initLightbox(); }catch(e){ console.error('init error', e); }
  });
})();